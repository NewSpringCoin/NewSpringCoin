...
unsigned int nTargetSpacing = 5 * 60; // 60 seconds
unsigned int nStakeMinAge = 7 * 24 * 60 * 60; // 7 days
unsigned int nStakeMaxAge = 4 * 7 * 60 * 60;  // 1 month / 4 weeks
...
static const int64_t nTargetTimespan = 5 * 60;  // 10 mins
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NewSpringCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Singapores first Prime Minister dies aged 91";
        CTransaction txNew;
        txNew.nTime = 1427077000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1427077000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1133797;
		if(fTestNet)
        {
            block.nNonce   = 1133797;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x874370a4dabe20d5297ea9847c552cb0080a2868a03ce9ca99487afc9e17f84d"));
        block.print();
